...
🟡 **Статус: частично выполнено** — `js/auth.js` декомпозирован и уменьшен до 416 строк, вынесены UI/API-части; `js/physics.js` декомпозирован через `js/physics-spawning.js`, `js/phaser/entities/EntityRenderer.js` декомпозирован через `js/phaser/entities/entity-render-passes.js` и выведен из oversized-списка; для `js/phaser/tunnel/TunnelRenderer.js` начата поэтапная декомпозиция (depth-light ray flow вынесен в `js/phaser/tunnel/tunnel-depth-rays.js`), но модуль ещё остаётся oversized.
...
🟡 **Статус: почти выполнено** — UI-составляющие auth вынесены в `js/auth-ui.js` (включая `renderAuthUiState`) и `js/auth-link-telegram-overlay.js`, API-вызовы вынесены в `js/auth-service.js`, в `js/auth-state.js` вынесены контейнер состояния + селекторы/мутации сессии, callback-оркестрация вынесена в `js/auth-callbacks.js`, wallet-подпись вынесена в `js/auth-wallet-connector.js`, auth-вход вынесен в `js/auth-authentication.js`, Telegram-env хелперы вынесены в `js/auth-telegram.js`, link-флоу вынесены в `js/auth-linking.js`, lifecycle-хелперы старта/дисконнекта вынесены в `js/auth-lifecycle.js`; в `js/auth.js` осталась тонкая orchestration-склейка экспортов.
...
✅ **Статус: выполнено** — в `scripts/check-static-analysis.mjs` добавлены burn-down milestones с датами/лимитами и fail при превышении активной вехи.
