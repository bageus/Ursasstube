...
🟡 **Статус: частично выполнено** — UI-составляющие auth вынесены в `js/auth-ui.js` и `js/auth-link-telegram-overlay.js`; orchestration и сервисная логика остаются в `js/auth.js` для следующего шага декомпозиции.
