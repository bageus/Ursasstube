...
### P0.1 Mobile Rendering & Tube Quality

**Проблема:**
- Труба остаётся самым тяжёлым элементом рендера.
- На мобильных — просадка FPS/качества, перегрев и повышенный расход батареи.

**Что делаем:**
- Вводим пресеты качества:
  - `low` — минимальные эффекты;
  - `medium` — сбалансированный режим;
  - `high` — полный визуал.
- Отдельный mobile-path рендера:
  - уменьшаем число слоёв (grid/fx/depth);
  - понижаем плотность геометрии;
  - условно отключаем тяжёлые VFX.

**KPI / критерии готовности:**
- стабильные 55–60 FPS на целевых мобильных устройствах;
- отсутствие резких thermal-throttling просадок в 5–10 минутной сессии;
- визуальная читаемость трубы не деградирует в `medium`.
