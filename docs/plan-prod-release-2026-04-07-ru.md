...
- Начат Epic A2: добавлены first-run onboarding hints в gameplay-start (`Swipe to change lane`, `Collect bonuses, avoid obstacles`) с одноразовым показом через localStorage-флаг и telemetry-событием `onboarding_hint_shown`.
