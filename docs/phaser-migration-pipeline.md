...
1. Выделить **render snapshot contract** — единый объект данных, который будет получать рендер.
2. Описать минимальный набор данных для первого этапа:
   - размеры viewport;
   - состояние трубы;
   - состояние игрока;
   - массив препятствий;
...
3. Зафиксировать, какие поля являются источником истины в логике и не должны вычисляться повторно в Phaser.
4. Определить feature flag:
   - `canvas`
   - `phaser`
5. Зафиксировать список текущих визуальных фич трубы, которые должны сохраниться после первой миграции.
