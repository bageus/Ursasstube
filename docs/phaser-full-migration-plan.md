...
- [x] **Этап 1/5 (частично):** physics/input больше не зависят от `DOM.canvas` для координат центра/проекции; используется единый viewport helper.
- [x] **Этап 5 (частично):** удалён legacy-модуль `js/renderer.js`; проекция/анимация игрока вынесены в узкий gameplay-only модуль `js/gameplay-projection.js`.
