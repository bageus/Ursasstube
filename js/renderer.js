...
class TubeRenderer {
  draw() {
    const tubeStart = performance.now();
    ...
    for (let d = CONFIG.TUBE_DEPTH_STEPS - 1; d >= 0; d -= depthStep) {
      ...
      const bevelLightStyle = `rgba(255, 225, 235, ${(0.24 * bevelDepthFade).toFixed(3)})`;
      const bevelDarkStyle = `rgba(10, 0, 14, ${(0.32 * bevelDepthFade).toFixed(3)})`;
      const groutStyle = `rgba(6, 0, 8, ${(0.26 * bevelDepthFade).toFixed(3)})`;
      const insetFillStyle = `rgba(0, 0, 0, ${(0.15 * bevelDepthFade).toFixed(3)})`;
      const rimStrokeStyle = `rgba(255, 180, 210, ${(0.12 * bevelDepthFade).toFixed(3)})`;
       ...
       for (let i = 0; i < CONFIG.TUBE_SEGMENTS; i += segmentStep) {
        ...
        ctx.strokeStyle = rimStrokeStyle;
        ctx.lineWidth = 0.9;
        ctx.beginPath();
        ctx.moveTo(rx1, ry1);
        ctx.lineTo(rx2, ry2);
    ...
    gameState.debugStats.tubeMs = performance.now() - tubeStart;
  }
}
