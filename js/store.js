...
function buildTelegramDonationStarsPayload(product) {
  ...
  const telegramInitData = String(webApp?.initData || '').trim();
...
async function handleTelegramDonationBuy(product) {
  ...
  const headers = {
    ...
    'X-Telegram-Init-Data': String(requestPayload.telegramInitData || '').trim()
  };
  ...
  donationPaymentState.payment = {
    ...
    title: data.title || product.title || product.key,
    amount: data.amount ?? displayPrice.amount,
