...
const BASE_CONFIG = Object.freeze({
  SPEED_START: 0.04,
  SPEED_MAX: 0.12,
  CAMERA_SHAKE_SMOOTHING: 12
});

function inlineReference({ gameState, adaptiveProfile, config, delta, randomX, randomY }) {
...
test('matches the current inline camera shake calculation across a reference matrix', () => {
  const cases = [
    { gameState: BASE_STATE, adaptiveProfile: BASE_PROFILE, config: BASE_CONFIG, delta: 1 / 60, randomX: 0.9, randomY: 0.1 },
    { gameState: { ...BASE_STATE, speed: 0.05 }, adaptiveProfile: BASE_PROFILE, config: BASE_CONFIG, delta: 1 / 60, randomX: 1, randomY: 0 },
    { gameState: { ...BASE_STATE, distance: 1999 }, adaptiveProfile: { tier: 'beginner' }, config: BASE_CONFIG, delta: 1 / 60, randomX: 0.8, randomY: 0.2 },
    { gameState: { ...BASE_STATE, distance: 2000 }, adaptiveProfile: { tier: 'beginner' }, config: BASE_CONFIG, delta: 0.2, randomX: 0.7, randomY: 0.3 }
  ];
...
test('suppresses shake for protected non-standard tiers before 2000m', () => {
  const result = calculateCameraShakeStep({
...
test('does not mutate grouped inputs', () => {
  ...
  calculateCameraShakeStep({ gameState, adaptiveProfile, config, delta: 1 / 60, randomX: 0.8, randomY: 0.2 });
