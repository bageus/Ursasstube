...
Progress note (2026-03-23): Centralized gameplay run/transient resets in `js/state.js`, added explicit auth/store snapshot and mutation helpers, and removed stale auth compatibility exports that were no longer imported anywhere. Stage 6 is now complete; the next step should start with Stage 7 dead-code and public API cleanup.
