...
import { project, projectPlayer, updatePlayerAnimation } from './projection.js';
...
function getViewportSize() {
  return {
    width: DOM.gameViewport?.clientWidth || window.innerWidth || 360,
    height: DOM.gameViewport?.clientHeight || window.innerHeight || 640
  };
}
...
function update(delta) {
  ...
  for (let i = gameState.collectAnimations.length - 1; i >= 0; i--) {
    const effect = gameState.collectAnimations[i];
    effect.timer = Math.max(0, (effect.timer || 0) - delta);
    if (effect.timer <= 0) {
      gameState.collectAnimations.splice(i, 1);
    }
  }
  ...
  const p = projectPlayer(CONFIG.PLAYER_Z, viewportSize);
  ...
  const playerPos = magnetActive ? projectPlayer(CONFIG.PLAYER_Z, viewportSize) : null;
  ...
  for (let i = coins.length - 1; i >= 0; i--) {
    ...
    // Magnet
     if (magnetActive && playerPos && c.z > 0.05 && c.z < 1.5) {
      const cp = typeof c.lane === "number" ? project(c.lane, c.z, viewportSize) : null;
     if (cp) {
...
function applyBonus(bonus) {
  const p = project(bonus.lane, bonus.z, getViewportSize());
  if (p) {
...
function collectCoin(coin) {
  ...
  if (coin.lane !== undefined) {
    const p = project(coin.lane, coin.z, getViewportSize());
