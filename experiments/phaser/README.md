...
## Post-release quick checklist

- Console has no Phaser/runtime errors during a full run + restart cycle.
- FPS on low-end mobile viewport is stable against pre-release baseline.
- Critical scenarios verified: lane switch, spin, collisions, bonuses, coins, game over, restart.

## Repository merge guide

If you need to merge the old `bageus/Phaser` repository into this main repo, run the flow **from this main repo** and follow `docs/phaser-repo-merge.md` for exact subtree import + transfer steps and post-merge checks.
...
- Legacy canvas renderer pipeline (`js/renderer.js`) has been removed. Runtime rendering now goes through Phaser backend only.
